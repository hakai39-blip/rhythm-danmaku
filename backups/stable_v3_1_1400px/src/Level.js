import { Bullet, Beam, Piston, RainBullet, Snake, Bomb, PulseBackground, TrailBullet, RadialWarning, LaneGuide } from './Hazards.js?v=3.1';
...
export class Level {
    ...
    update(dt) {
        ...
        // Level 1: 56 Measures
        // Level 2: Was 64. Music finishes earlier or has long tail. 
        // User requested cutting 8 seconds. 8 sec @ 180BPM approx 6 measures.
        // 64 - 6 = 58. Let's try 58 for Level 2.
        const maxMeasures = (this.levelId === 1) ? 56 : 57;

        // Also if audio stops, trigger end timer immediately?
        // Logic: If measure reached OR audio stopped naturally?
        // User said "After song ends... 10s silence".
        // This implies the audio file itself has silence.
        // So checking `audio.isPlaying` might not help if it's playing silence.
        // We rely on `maxMeasures`.

        if (audio.measure >= maxMeasures && !this.gameFinished) {
            if (audio.isPlaying) audio.stop();
            this.endTimer += dt;
            if (this.endTimer > 1.0) { // Reduced wait from 3.0 to 1.0 for snappier finish
