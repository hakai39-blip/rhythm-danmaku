export class AudioController {
    ...
    setSong(id) {
        ...
        else if (id === 3) this.bpm = 120; // Song 3

        if (id === 1) this.totalMeasures = 57;
        else if (id === 2) this.totalMeasures = 64;
        else if (id === 3) this.totalMeasures = 48; // Shorter loop for now
    }
    ...
    pause() {
        // Do NOT suspend context, so SFX can finish playing naturally.
        // Just stop the scheduler to pause the music progression.
        this.isPlaying = false;
        clearTimeout(this.timerID);
    }

    resume() {
        // Ensure context is running (in case browser auto-suspended)
        if (this.ctx && this.ctx.state === 'suspended') this.ctx.resume();

        this.isPlaying = true;
        // Re-anchor the music timer to NOW so we don't fast-forward
        this.nextNoteTime = this.ctx.currentTime + 0.1;
        this.scheduler();
    }
    ...
    scheduleNote(beatNumber, time) {
        ...
        else if (this.currentSongId === 3) this.scheduleSong3(beatNumber, time);
    }

    // ==========================================
    // SONG 3: Cavern Depths (Level 1-3)
    // ==========================================
    scheduleSong3(beatNumber, time) {
        const m = this.measure;
        const step = beatNumber;
        if (m >= 48) return;

        // Structure:
        // 0-8: Intro (Drip drops, eerie pads)
        // 8-24: Main Beat (Slow, heavy industrial)
        // 24-32: Fast Section (Panic)
        // 32-48: Finale (Heavy + Chimes)

        const isIntro = m < 8;
        const isMain = m >= 8 && m < 24;
        const isPanic = m >= 24 && m < 32;
        const isFinale = m >= 32;

        // -- DRUMS --
        if (isMain || isFinale) {
            // Heavy Kick on 1 and 3
            if (step === 0 || step === 8) this.playKick(time, 1.5);
            // Snare with reverb feeling (noise)
            if (step === 4 || step === 12) this.playSnare(time);
        }
        if (isPanic) {
            // Double time feel
            if (step % 4 === 0) this.playKick(time, 1.2);
            if (step % 4 === 2) this.playSnare(time);
        }

        // -- HIHATS --
        if (m >= 4) {
            if (step % 2 === 0) this.playHiHat(time, 0.2);
            if (step % 4 === 2) this.playHiHat(time, 0.4, true);
        }

        // -- BASS --
        if (!isIntro) {
            // C Minor: C(65.4) -> G(98) -> Eb(77.7)
            let freq = 65.4;
            if (m % 4 === 1) freq = 77.7; // Eb
            if (m % 4 === 2) freq = 58.2; // Bb
            if (m % 4 === 3) freq = 49.0; // G

            if (step === 0) this.playHeavySynth(time, freq, 0.6);
            if (step === 8) this.playHeavySynth(time, freq, 0.4);
        }

        // -- FX / MELODY --
        if (isIntro) {
            // Random water drips
            if (Math.random() < 0.2) this.playSine(time, 1000 + Math.random() * 500, 0.3);
        }

        if (isMain || isFinale) {
            // Industrial metallic hit
            if (step === 0 && m % 2 === 0) this.playSquare(time, 110, 0.3);
        }

        if (isFinale || isPanic) {
            // Arp
            const arps = [261, 311, 392, 523]; // Cm
            if (step % 2 === 0) {
                const note = arps[(step / 2) % 4];
                this.playSaw(time, note, 0.15);
            }
        }
    }
